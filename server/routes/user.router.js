...
router.post('/', function (req, res) {
    if (req.isAuthenticated()) {
        console.log('post request user access', req.user);
        pool.connect(function (connectionError, client, done) {
            ...
            console.log('this is toDo.item', toDo.item);

            if (connectionError) {
                console.log(connectionError);
                res.sendStatus(501);
            } else {
                var pQuery = 'INSERT INTO list (name,users_id) VALUES ($1,$2);'; 
                var toDoArray = [toDo.listName,toDo.userId];
