...
router.get('/',function(req,res){
    ...
    pool.connect(function(connectionError, client, done){
        if(connectionError) {
            ...
            var queryString = 'SELECT list.name, array_agg(items.item)AS item FROM list INNER JOIN items ON items.list_id=list.id WHERE list.users_id = $1 GROUP BY list.name';
